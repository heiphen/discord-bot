        ...
        if (newUser) {
          ...
          for(let i=0;i<newUser.userIds.length;i++) {
            ...
            if (!userIdInArray) {
                ...
                if (roleToAssign) {
                  userRoleCol.findOneAndUpdate({code: message.content},{$set: {userIds: [...newUser.userIds, message.author.id]}})
                    const role = message.guild?.roles.cache.find(role => role.id === roleToAssign);
